...
parser.add_argument('--resume', '-r', default='/root/_VCM_lr_0.1_batchsize_8v2t_rank1_best.t', type=str,
...
def test1():
    ...
    gall_feat = np.zeros((ngall, 2048))  # ngall = 5099
    q_pids, q_camids = [], []
    g_pids, g_camids = [], []
    with torch.no_grad():
    ...
    # np.savetxt("rgb_pids.txt", g_pids, fmt='%d', delimiter=',')
    ...
    # np.savetxt("ir_pids.txt", q_pids, fmt='%d', delimiter=',')
    ...
    # evaluation
    cmc, mAP = evaluate_test(-distmat, q_pids, g_pids, q_camids, g_camids)
...
def test2():
    ...
    with torch.no_grad():
        for batch_idx, (imgs, pids, camids) in enumerate(galleryloader_1):
            ...
            g_camids.extend(camids)
    g_pids = np.asarray(g_pids)
    # np.savetxt("v2t_g_pids.txt", g_pids, fmt='%d', delimiter=',')
    g_camids = np.asarray(g_camids)
    ...
    q_pids = np.asarray(q_pids)
    # np.savetxt("v2t_q_pids.txt", q_pids, fmt='%d', delimiter=',')
    q_camids = np.asarray(q_camids)
    print('Extracting Time:\t {:.3f}'.format(time.time() - start))
